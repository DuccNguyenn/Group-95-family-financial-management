    ...
    <div className="flex h-screen w-full bg-[#f8faf9] dark:bg-[#0f1a14] overflow-hidden">
      ...
      <aside className="w-64 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-[#122017] flex flex-col hide-scrollbar">
        ...
        <div className="h-16 flex items-center gap-3 px-6 border-b border-slate-100 dark:border-slate-800/50">
          ...
          <div>
            <h1 className="font-bold text-sm text-slate-800 dark:text-slate-100">Gia Kế Admin</h1>
            <p className="text-[10px] text-slate-500">Quản trị viên hệ thống</p>
          </div>
        </div>

        {/* Navigation */}
        <div className="flex-1 overflow-y-auto py-6 px-4 flex flex-col gap-1">
          <div className="text-xs font-semibold text-slate-400 dark:text-slate-500 px-2 mb-2 uppercase tracking-wider">
            Quản lý chung
          </div>
          {menu.map((item) => {
        ...
        <div className="p-4 border-t border-slate-100 dark:border-slate-800/50">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2 px-4 rounded-xl text-sm font-medium text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors"
          >
        ...
        <header className="h-16 bg-white dark:bg-[#122017] border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-8 shadow-sm">
          ...
          <div className="flex items-center gap-3">
             <div className="text-right hidden sm:block">
               <div className="text-sm font-medium text-slate-700 dark:text-slate-200">{user?.name || 'Admin'}</div>
